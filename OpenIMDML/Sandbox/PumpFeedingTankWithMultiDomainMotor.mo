...
model PumpFeedingTankWithMultiDomainMotor
  "A modified version of the tank filled by mechanical pump from Dassault Systemes Cooling Library"
  ...
  inner Cooling.Common.SystemSettings systemSettings(allowFlowReversal=true)
  ...
  MultiDomainModels.Motors.MD_ALL_IN_ONE_ThreePhaseMotor motor(
    M_b=15000000,
    Sup=true,
    redeclare OpenIMDML.MultiDomainModels.Motors.ThreePhase.PSAT.MD_MotorTypeI
      motor)
...
equation
  ...
  connect(Torque_Sensor.flange_a, motor.flange1)
    annotation (Line(points={{12,64},{-6,64}}, color={0,0,0}));
  connect(Torque_Sensor.flange_b, Load_Inertia.flange_a)
    annotation (Line(points={{32,64},{76,64}}, color={0,0,0}));
