...
model MultiDomainMotorCIMStartUpComparison
  ...
  Modelica.Blocks.Sources.RealExpression TorqueEquation(y=-(0.1*(15/100)*(
        CIM_with_VSD.s) + 0.5*(15/100)*(1 - CIM_with_VSD.s)^2)*CIM_with_VSD.T_b)
  ...
  MultiDomainModels.Motors.ThreePhase.PSSE.CIM5_CIM6 CIM_with_VSD(V_b=23000,
      Mtype=2) annotation (Placement(transformation(extent={{80,40},{60,60}})));
...
equation
  ...
  connect(AC2DC_and_DC2AC.n, CIM_with_VSD.p)
    annotation (Line(points={{48,50},{60,50}}, color={0,0,255}));
  connect(CIM_with_VSD.flange, torqueSensor3.flange_a)
    annotation (Line(points={{80,50},{92,50}}, color={0,0,0}));
  connect(torqueSensor3.tau, CIM_with_VSD.mech_torque) annotation (Line(points={
          {94,39},{94,32},{76,32},{76,38}}, color={0,0,127}));
  connect(VfController.we, CIM_with_VSD.we)
    annotation (Line(points={{50,-4},{70,-4},{70,38}}, color={0,0,127}));
  connect(CIM_with_VSD.wr, VfController.motor_speed)
...
<p>The example MultiDomainMotorTypeCIMStartUpComparison contains two separate systems meant for comparison. The top system is a multi-domain three-phase type V induction motor model controlled by a Variable Speed Drive while the bottom system is the same example but with no VSD controller.
...
<ul>
<li><code>CIM.Imag vs CIM_with_VSD.Imag</code></li>
<li><code>CIM.s vs CIM_with_VSD.s</code></li>
<li><code>CIM.wr vs CIM_with_VSD.wr</code></li>
<li><code>CIM.P vs CIM_with_VSD.P</code></li>
<li><code>CIM.Q vs CIM_with_VSD.Q</code></li>
